...
## snakemake -np -s workflow/preprocessing.smk --profile workflow/demux_align
## snakemake -s workflow/preprocessing.smk --profile workflow/demux_align --dag | dot -Tpng > preprocessing_dag.png
...
rule all:
    ...
    input: 
        expand(f"{w_dir}/{{sample}}.aligned.sort.bam.csi", sample = sample_names),
        expand(f"{w_dir}/{{sample}}.5mC.6mA.aligned.nuc.bam.csi", sample = sample_names)
...
## Filter modified bases: As suggested by FIRE repo: Omit lower 10%Q of modified bases called.
rule filter_and_nucleosomes:
    input: "{w_dir}/{sample}.aligned.sort.bam"
    output: "{w_dir}/{sample}.5mC.6mA.aligned.nuc.bam"
    params:
        sam_dir = "{w_dir}/{sample}",
    resources:
        mem_mb = 185000,
        ntasks_per_node = 8,
    shell:
        ...
        echo "Temporary bam file: {params.sam_dir}/tmp.bam"
        ## Temporary bam file, to avoid pipe issues.
        modkit call-mods -t 8 -p 0.1  --log-filepath {params.sam_dir}/modkit_filter.log {input} {params.sam_dir}/tmp.bam
        ft add-nucleosomes {params.sam_dir}/tmp.bam {output}
        rm {params.sam_dir}/tmp.bam
        """

rule index_2:
    input: "{w_dir}/{sample}.5mC.6mA.aligned.nuc.bam"
    output: "{w_dir}/{sample}.5mC.6mA.aligned.nuc.bam.csi"
    resources:
        mem_mb = 50000,
        ntasks_per_node = 8,
    shell:
